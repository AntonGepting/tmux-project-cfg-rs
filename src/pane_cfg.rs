...
use self::tmux_interface::{
    PaneSize, Panes, SelectPane, SplitWindow, TargetPaneEx, TargetPaneToken, TargetWindowEx,
    TmuxInterface,
};
...
impl PaneCfg {
    ...
    pub fn create(&self, target_window: &TargetWindowEx) -> Result<usize, Error> {
        // target pane is next (+1) from current
        let target_pane = TargetPaneEx::token(Some(&target_window), TargetPaneToken::Next(None));
        ...
        if let Some(value) = first_value {
            ...
            if let Some(size) = value.size {
                l = PaneSize::Size(size);
                split_window.size = Some(&l);
            };
            if let Some(percentage) = value.percentage {
                p = PaneSize::Percentage(percentage);
                split_window.size = Some(&p);
            };
            split_window.target_pane = Some(&target_pane);
        ...
        send_keys.and_then(|k| k.send(&TargetPaneEx::id(Some(&target_window), id)).ok());
        Ok(id)
    }

    pub fn select(target_pane: &TargetPaneEx) -> Result<(), Error> {
        // init tmux interface
        let mut tmux = TmuxInterface::new();
    ...
    pub fn get(
        target_window: &TargetWindowEx,
        index: usize,
        bitflags: usize,
    ) -> Result<PaneCfg, Error> {
        ...
        for pane in panes {
            // unwrap or error
            // compare depending on enum variant
            if pane.index == Some(index) {
                // TODO: from PaneOptions
